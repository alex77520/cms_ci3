...
<section class="page-category page-category__history">
		...
		<h3><span class="item_title_part0 item_title_part_odd item_title_part_first_half item_title_part_first">Categoria: <?php echo $category_title->name_cat; ?></span> </h3>	
		</header>
</section>
	
<div class="items-row__ cols-2 row-0_ row-fluid______">
<?php if(!empty($category_slug)): ?>
<?php foreach($category_slug as $row){ ?>
...
<?php } ?> 
    <?php else: ?>
        <p>No se encuentra ningun noticia relacionado a este categoría.</p>
    <?php endif; ?>
</div>
