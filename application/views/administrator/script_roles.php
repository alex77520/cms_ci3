...
function add_rol()
{
    ...
    $('.modal-title').text('Añadir Rol'); // Set Title to Bootstrap modal title
}

function edit_rol(id_rol)
{
    save_method = 'update';
    $('#form')[0].reset(); // reset form on modals
    $('.form-group').removeClass('has-error'); // clear error class
    ...
    $.ajax({
        url : "<?php echo site_url('roles/ajax_edit/')?>/" + id_rol,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('.modal-title').text('Editar Rol'); // Set title to Bootstrap modal title
...
function save()
{
    ...
    $.ajax({
        ...
        error: function (jqXHR, textStatus, errorThrown)
        {
            alert('Error añadiendo / actualizando data');
            $('#btnSave').text('Guardar'); //change button text
            $('#btnSave').attr('disabled',false); //set button enable 
